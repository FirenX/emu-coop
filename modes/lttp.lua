...
local trValue = 0x18
local gtValue = 0x1A
local izValue = 0x05 --invisible zelda
local haveIzelda = false

return {
guid = "fe302609-5d04-4f17-ab56-f358f8a6ad8d",
	format = "1.1",
	...
	sync = {
		...
		--fix for bitchy invis zelda after getting to her in HC basement
		[0x7EF3CC] = {kind=function(value, previousValue)
				allow=false
				if value == izValue then
					haveIzelda=true
				end
				if value == sewerValue and haveIzelda then
					allow=true
					haveIzelda=false		
				end
				return allow, value	 -- if so use the new value
			end
		},
